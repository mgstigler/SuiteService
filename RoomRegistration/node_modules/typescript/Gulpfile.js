...
gulp.task(tsserverLibraryFile, false, [servicesFile], (done) => {
    ...
    const { js, dts } = gulp.src(languageServiceLibrarySources)
        .pipe(sourcemaps.init())
...
gulp.task("runtests", "Runs the tests using the built run.js file. Optional arguments are: --t[ests]=regex --r[eporter]=[list|spec|json|<more>] --d[ebug]=true --color[s]=false --lint=true.", ["build-rules", "tests"], (done) => {
...
const tslintRulesOutFiles = tslintRules.map(function (p, i) {
    ...
    gulp.task(pathname, false, [], () => {
        const settings = getCompilerSettings({ module: "commonjs" }, /*useBuiltCompiler*/ false);
