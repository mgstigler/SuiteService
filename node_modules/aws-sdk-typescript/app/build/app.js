...
function readServiceFiles() {
    Object.keys(metadata).forEach(function (shortName) {
...
function generateServiceDefinitions() {
    Object.keys(metadata).forEach(function (serviceName) {
...
function copyCommonDefs() {
    ...
    ['output', 'output/typings'].forEach(function (path) {
...
function generateModuleFile() {
    var services = Object.keys(metadata)
        .map(function (shortName) { return metadata[shortName].prefix; });
...
function cleanDefinitions() {
    var unrecognizedServices = Object.keys(metadata)
        .filter(function (name) { return !metadata[name].descriptor; });
    unrecognizedServices.forEach(function (name) {
            ...
            estraverse.traverse(ast, {
                enter: function (n) {
                    if (matcher(n)) {
                        ...
                        var methodList = n.arguments[1].properties.map(function (x) { return x.key.name; });
